...
// Image-based feature card with hover effects (IDs 1 and 4)
const ImageFeatureCard = ({ title, description, imagePath }: Omit<FeatureCardProps, 'icon' | 'highlight' | 'id'>) => {
...
// Content-based feature card (IDs 2 and 3)
const ContentFeatureCard = ({ title, description, icon, highlight }: Omit<FeatureCardProps, 'imagePath' | 'id'>) => {
  return (
    <div 
...
// Feature card that determines which type to render based on ID
const FeatureCard = ({ title, description, icon, imagePath, highlight, id }: FeatureCardProps) => {
  // Cards with IDs 1 and 4 show image with bullet points
  const isImageCard = id === 1 || id === 4;
  
  if (isImageCard) {
    return <ImageFeatureCard title={title} description={description} imagePath={imagePath} />;
  }
  
  return <ContentFeatureCard title={title} description={description} icon={icon} highlight={highlight} />;
};

// ROI metrics card with Apple-inspired design
...
// Company card with simplified design
...
const HrRoiComponent = () => {
  ...
  // Render mobile features (only visible on smaller screens)
